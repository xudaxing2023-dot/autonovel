...
def build_chapter_prompt(
    ...
    return f"""请撰写第 {chapter_num} 章，并为本章起一个吸引人的章节标题。

格式要求：正文第一行必须是 `# 第{chapter_num}章：标题`（例如 `# 第3章：深渊之眼`）。
标题 4-12 字，应暗示本章核心事件或情感基调，但避免直接剧透关键转折。
...
19. 【章节标题】: 第一行格式为 `# 第{chapter_num}章：标题`。
    标题 4-12 字，暗示本章核心但不剧透。
