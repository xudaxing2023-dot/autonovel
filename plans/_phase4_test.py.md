...
def run_pipeline(mode: str = "from_scratch", timeout_minutes: int = 120) -> tuple:
    """
    以子进程方式运行 pipeline_orchestrator。
    返回 (exit_code, elapsed_seconds, output_lines)。
    """
    print(f"\n{'=' * 70}")
    print(f"  🚀 启动流水线: pipeline_orchestrator --mode {mode}")
    print(f"{'=' * 70}")
    ...
    return result.returncode, elapsed, lines
...
def verify_file(path: Path, min_bytes: int = 100,
    ...
    return True, f"{label} ✓ ({size} bytes)"


def print_results(results: list, test_name: str, elapsed: float) -> None:
    """打印测试结果汇总。"""
    print(f"\n{'─' * 60}")
    print(f"  📋 {test_name} 结果 ({elapsed:.0f}s)")
    print(f"{'─' * 60}")
    passed = 0
    failed = 0
...
def test_3_resume_recovery() -> list:
    ...
    results.append(("Resume 无from_scratch横幅",
                    ...
                    "未出现" if not from_scratch_banner else "❌ 出现了"))
    ...
    try:
        ...
        if BACKUPS.exists():
            ...
            for snap in snapshots:
                ...
                print(f"  📦 {snap.name}: label={'✓' if has_label else '✗'}, "
...
推荐执行顺序: 4 → 3 → 1 → 2
  (4 先跑因为它涉及 .git 操作，需要确保能恢复)
    ...
    print("\n" + "█" * 70)
    print(f"  📊 阶段4 测试汇总 ({overall_elapsed/60:.1f}min)")
    print("█" * 70)
    ...
    for test_name, res_list in all_results.items():
        ...
        grand_pass += p
        grand_fail += f
        print(f"  {test_name}: {p}✅ {f}❌ ({p+f} 项)")

    print(f"  ────────")
    print(f"  总计: {grand_pass}✅ {grand_fail}❌ ({grand_pass + grand_fail} 项)")
